...
class Button:
    ...
    def mouse_down(self):
        if (self.posX - 4 < pygame.mouse.get_pos()[0] < self.posX - 4 + self.width and
            self.posY - 4 < pygame.mouse.get_pos()[1] < self.posY - 4 + self.height):
...
play_button = Button('C:\\Users\\Matty\\Desktop\\play.png', 145, 250, 200, 188)  # 設定「開始」按鈕的位置跟image
restart_button = Button("C:\\Users\\Matty\\Desktop\\restart.png", 180, 260, 100, 100)  # 設定「重新開始」按鈕的位置跟image
switch_button = Button("C:\\Users\\Matty\\Desktop\\switchIcon.png", 400, 450, 50, 50)  # 設定「重新整理」道具按鈕的位置跟image
delcolor_button = Button("C:\\Users\\Matty\\Desktop\\brush.png", 350, 450, 50, 50)  # 設定「消除隨機一色的所有方塊」道具按鈕的位置跟image
...
while True:  # 開始介面的PLAY按鈕
    ...
    if event.type == QUIT:
        ...
        if play_button.get_mouse_on_button() == 'Yes':
        ...
        if event.key == K_SPACE:
            startIcon = pygame.Surface([200, 180])
            startIcon.fill([0, 0, 0])
...
while True:
    while True:
        ...
        if event.type == QUIT:
            ...
            switch_button = Button("C:\\Users\\Matty\\Desktop\\商管程式設計\\wee\\switchIcon.png", 400, 450, 50, 50)  # 設定「重新整理」道具按鈕的位置跟image
            delcolor_button = Button("C:\\Users\\Matty\\Desktop\\商管程式設計\\wee\\brush.png", 350, 450, 50, 50)  # 設定「消除隨機一色的所有方塊」道具按鈕的位置跟image
